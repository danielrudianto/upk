...
TransactionController.create = (req, res) => {
    ...
    */
    ...
    payment_model_1.default.fetchById(paymentMethod)
        .then((payment) => {
        if (payment == null || payment.is_delete) {
            ...
            try {
                axios_1.default
                    ...
                    const user_transaction = new transaction_model_1.default(req.body.userId, parseFloat(result.data.HARGA) + parseFloat(result.data.ADMIN), parseInt(process.env.ADMINISTRATION_FEE), parseFloat(result.data.KOMISI), productCodeName, purchaseReference, result.data.REF2, paymentMethod, req.body.districtId, req.body.note);
                    user_transaction
                        .create()
                        .then((transaction) => {
                        const value = parseFloat(transaction.value.toString());
                        const discount = parseFloat(transaction.discount.toString());
                        ...
                        switch (payment.id) {
                            case 1:
                                ...
                                bank_service_1.default.createVirtualAccount(transaction.user.id.toString().padStart(10, "0"), transaction.user.name, value + service - discount, `Transaksi AbangKu #${transaction.id.toString().padStart(10, "0")}`, expired)
                                    .then((result) => {
                                    ...
                                    res
                                        .status(500)
                                        .send("Gagal membuat billing, mohon coba kembali dalam beberapa saat.");
                                });
                                break;
                        }
                    })
