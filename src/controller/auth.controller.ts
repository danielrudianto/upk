...
class authController {
  static register = (req: Request, res: Response, next: NextFunction) => {
    ...
    const user_by_phone_number = UserModel.getUserByPhoneNumber(
      formatted_phone_number
    );

    QueryTransactionHelper.create([user_by_nik, user_by_phone_number])
    ...
    if (!req.body.user_management_id) {
      ...
      QueryTransactionHelper.create([
        ...
        .then(async (result) => {
          ...
          if (!(await compare(req.body.password.toString(), user.password))) {
            return res
              .status(404)
              .send(
                "Nomor telepon atau password salah, mohon periksa kembali."
